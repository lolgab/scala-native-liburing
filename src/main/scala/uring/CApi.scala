...
object CApi {
  ...
  def timerfd_settime(fd: CInt, flags: CInt,new_value: Ptr[itimerspec], old_value: Ptr[itimerspec]): CInt = extern
  ...
  def io_uring_prep_poll_remove(sqe: Ptr[Byte], user_data: RawPtr): Unit = extern
  ...
  def io_uring_prep_timeout_remove(sqe: Ptr[Byte], user_data: RawPtr, count: CInt): Unit = extern
  ...
  def io_uring_cqe_get_data(cqe: Ptr[Byte]): RawPtr = extern
  @name("scalanative_io_uring_sqe_set_data")
  def io_uring_sqe_set_data(sqe: Ptr[Byte], data: RawPtr): Unit = extern
